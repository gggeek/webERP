...
if ($NoOfGRNs >0){
	...
	for ($i=1;$i<=$NoOfGRNs;$i++) {
		...
		if($FooterPrintedInPage == 0){
			$pdf->addText($FormDesign->ReceiptDate->x,$Page_Height-$FormDesign->ReceiptDate->y,$FormDesign->ReceiptDate->FontSize, _('Date of Receipt: ') . $DeliveryDate);
			$pdf->addText($FormDesign->SignedFor->x,$Page_Height-$FormDesign->SignedFor->y,$FormDesign->SignedFor->FontSize, _('Signed for').' ______________________');
		...
		*/
		...
		if ($ControlledRow[0]==1) { /*Then its a controlled item */
			...
			while ($SerialStockMoves = DB_fetch_array($GetStockMoveResult)){
				$pdf->addTextWrap($FormDesign->Data->Column1->x-20,$Page_Height-$YPos,$FormDesign->Data->Column1->Length,$FormDesign->Data->Column1->FontSize, _('Lot/Serial:'),'right');
